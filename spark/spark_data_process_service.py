...
# MUDANÇA: URL base para buscar dados do silo (ex: /silos/1)
...
# MUDANÇA: Limites removidos daqui, serão buscados da API
    ...
    while True:
        try:
            ...
            if res.status_code == 200:
                ...
                print(f"⚠️ Erro ao verificar silo #{silo_id}: {res.status_code}")
        except Exception as e:
            print(f"❌ Falha ao checar silo #{silo_id}: {repr(e)}")
        time.sleep(CHECK_INTERVAL)

# ======================================
# FUNÇÃO DE PROCESSAMENTO (COM CORRELAÇÃO)
# ======================================
def process_device(device_key: str, silo_id: int, min_timestamp: int, silo_config: dict):
    ...
    try:
        ...
        if not raw_data:
            print(f"⚠️ Nenhum dado novo em {device_key}")
            return
        ...
        for record_json in raw_data:
            ...
            if isinstance(payload_data, str):
                try:
                    ...
                    print(f"⚠️ Falha ao decodificar payload string: {payload_data}")
                    payload = {} 
        ...
        if not data:
            print(f"ℹ️ Nenhum dado válido para processar em {device_key} após o parse.")
            return
        ...
        grouped = df.groupBy(
            ...
            # ========================================================
            # MUDANÇA AQUI: Adicionando cálculos de correlação
            # ========================================================
        ...
        for row in grouped.collect():
            ...
            # ========================================================
            # MUDANÇA AQUI: Coletando os valores de correlação
            # ========================================================
            ...
            dto = {
                ...
                # ========================================================
                # MUDANÇA AQUI: Enviando a correlação Temp/Hum no alertsCount
                # ========================================================
                "alertsCount": corr_temp_hum,
                
                # Se quiser enviar as outras, mapeie para campos não utilizados:
            ...
            try:
                ...
                if 200 <= res.status_code < 300:
                    print(f"✅ [{device_key}] {period_start} → {period_end} | {res.status_code} (Salvo!)")
                else:
...
while True:
    ...
    for device_key in device_keys:
        ...
        if not silo_id:
            print(f"⚠️ {device_id} não mapeado para silo, ignorando...")
            continue
            
        # ========================================================
        # MUDANÇA: Buscar a configuração do silo
        # ========================================================
        silo_config = {}
        try:
            ...
            if res.status_code == 200:
                ...
                print(f"ℹ️ Configuração carregada para Silo #{silo_id} (Temp Max: {silo_config.get('maxTemperature')})")
            else:
                print(f"⚠️ Falha ao buscar config do silo #{silo_id} ({res.status_code}), usando padrões.")
        except Exception as e:
            print(f"❌ Erro ao buscar config do silo #{silo_id}: {repr(e)}, usando padrões.")
            
        # MUDANÇA: Passa o silo_config para a função
